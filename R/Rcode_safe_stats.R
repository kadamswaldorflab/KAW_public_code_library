...
safe_pairwise_tests <- function(df, x_var, g_var, subset_vars, testtype, ndigits, show_p_LT_point1)
{
  ...
  final_results <- sqldf(sqlcode)

  print(final_results %>% count(.test_status, pstars))
  return(final_results)
}



safe_pairwise_t_tests <- function(df, x_var, g_var, subset_vars, ndigits=3, show_p_LT_point1=F) {
  result <- safe_pairwise_tests(df, x_var, g_var, subset_vars, "T-test", ndigits, show_p_LT_point1)
  return(result)
} 

safe_pairwise_wilcox_tests <- function(df, x_var, g_var, subset_vars, ndigits=3, show_p_LT_point1=F) {
